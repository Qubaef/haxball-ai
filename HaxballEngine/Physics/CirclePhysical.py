...
class CirclePhysical(ABC):
    def __init__(
        ...
        self.hitbox: float = int(
            size * 3 / 2
        )  # radius of hitbox used for kicking the ball
        self.v_max = 8 / math.pow(
            self.weight, 2 / 3
        )  # maximum velocity is non-linear, cause ball was too fast
    ...
    def setVel(self, vel: pygame.Vector2) -> None:
        self.v = vel

    def addVel(self, vel: pygame.Vector2) -> None:
        self.v += vel * self.weight

    def setMovement(self, vel: pygame.Vector2, pos: pygame.Vector2) -> None:
    ...
    def update(self, dt: float) -> None:
        ...
        dt = dt * InternalProperties.TARGET_FPS
    ...
    def getCollisionCandidates(self) -> list:
        ...
        for i in range(
            int(self.p.x / InternalProperties.COLLISION_SECTOR_SIZE) - sectorNum,
            int(self.p.x / InternalProperties.COLLISION_SECTOR_SIZE) + sectorNum + 1,
        ):
            for j in range(
                int(self.p.y / InternalProperties.COLLISION_SECTOR_SIZE) - sectorNum,
                int(self.p.y / InternalProperties.COLLISION_SECTOR_SIZE)
                + sectorNum
                + 1,
            ):
                if 0 <= i < len(self.engine.collisionSectors) and 0 <= j < len(
                    self.engine.collisionSectors[0]
                ):
