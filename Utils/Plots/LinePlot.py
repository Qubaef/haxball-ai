...
class LinePlot(Plot):
    ...
    def __init__(self, name: str, xLabel: str, yLabels: List[str]):
        ...
        self.values: List[List, List[List]] = []
        self.values.append([])
        self.values.append([[] for _ in range(len(yLabels))])

    def storeVal(self, xVal: float, yVals: List[float]):
        assert len(yVals) == len(self.yLabels), "Number of y values must match number of y labels"
